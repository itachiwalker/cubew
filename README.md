...
| Cube state editor (<img src="icons/icon-set.svg" width="14" align="absmiddle">) | Tabbed (Settings / Exam / History). The Settings tab lets you read/write the cube's state as a 54-character kociemba-format string and run commands (with wildcard support: `W`/`w`/lowercase face letters). The History tab shows the initial state string and two move-command strings — plain, and one annotated with `CAM(θ,φ)` markers wherever the camera view changed — each shown truncated with a move count, and each with its own copy button (copies the full string). Also openable during a standalone Step-by-step run or Exam, showing History only (read-only, for grabbing progress mid-session) — Settings/Exam stay hidden then. While a SET → Auto-execute run or an external-solver solve is in progress, the stopwatch area shows "Auto-execute", the history nav bar shows real progress (done/total moves), and nearly every other control is locked out until it finishes |
...
| High scores | Saves your top 10 solve times after each scramble. The high score screen is shown automatically after clearing, with this run's time highlighted. Eligible entries (solved in 500 moves or fewer) show a ▶ button that replays that exact solve as a Step-by-step session, camera moves included |
...
| <img src="icons/icon-campin.svg" width="18" align="absmiddle"> | Pin the camera (shown while browsing freely/exams, or a standalone Step-by-step sequence using `CAM()`). When on, <img src="icons/icon-first.svg" width="14" align="absmiddle"><img src="icons/icon-back.svg" width="14" align="absmiddle"><img src="icons/icon-go.svg" width="14" align="absmiddle"><img src="icons/icon-last.svg" width="14" align="absmiddle"><img src="icons/icon-rev.svg" width="14" align="absmiddle"><img src="icons/icon-play.svg" width="14" align="absmiddle"> recreate the camera angle at the moment each move was actually made |
...
- **Sbs (Step-by-step)** replays a fixed, pre-built sequence. Every move and camera position is decided up front; afterward you can only browse it (step through, jump to start/end, autoplay at whatever speed) — not edit or interrupt it with your own moves. Good fit for: tutorials, replaying a recorded solution (e.g. a Highscore entry's ▶ button), practicing an algorithm move-by-move.
