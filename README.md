...
| Cube state editor (<img src="icons/icon-set.svg" width="14" align="absmiddle">) | Tabbed (Settings / Exam / History). The Settings tab lets you read/write the cube's state as a 54-character kociemba-format string and run commands (with wildcard support: `W`/`w`/lowercase face letters). The History tab shows the initial state string and two move-command strings — plain, and one annotated with `CAM(θ,φ)` markers wherever the camera view changed — each shown truncated with a move count, and each with its own copy button (copies the full string). Also openable during a standalone Step-by-step run or Exam, showing History only (read-only, for grabbing progress mid-session) — Settings/Exam stay hidden then. While a SET → Auto-execute run or an external-solver solve is in progress, the stopwatch area shows "Auto-execute" with the remaining move count, and swipes/double-tap commands are blocked until it finishes |
