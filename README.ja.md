...
| 試験モード（SET画面「試験」タブ） | 初期状態・ゴール状態を指定して自由に操作。初期状態欄はSET画面を開いた時点のキューブ状態で自動初期化。ゴール状態と一致すると自動でお祝い演出。「□ やり直す」ボタン（視点移動・キューブ操作のいずれかがあった場合のみ有効）で確認なしに初期状態へ即リセット可能。同じボタンはSbS実行中にも表示され、その場合は最初へ戻るボタンと同じ動作をする。RESET（⊞）はモード終了（確認ダイアログ経由）として機能 |
| ストップウォッチ | スクランブル後の最初の手動操作で自動開始・6面揃いで自動停止。停止時に順位表示（圏外含む）。白点滅中タップでハイスコア表示 |
| ハイスコア | スクランブル後に6面揃えたタイムと手数を上位10件保存。クリア後にハイスコア画面を自動表示・今回のタイムを強調。500手以内で揃えた場合、該当行に<img src="icons/icon-play.svg" width="14" align="absmiddle">ボタンが表示され、押すとその解法をステップ実行（視点移動・ダブルタップコマンド含む、最後はホーム視点で終了）として再生できる。再生中は、ベストタイムの表示位置にその解法のタイトル（日付・手数・タイム）が表示される。同じ行の共有ボタンからは、`cubew_replay_hub`（Supabase）を介した短いリンク（SHA-256先頭16文字をキーとする固定長URL）でリプレイを共有できる。共有データは30日で自動削除される |
...
| チュートリアル | LBL法の段階的レッスン・試験とF2L（全41パターン、開始状態画像は実装済み・ヒント画像は作成中）・用語解説の3レベルで構成。矢印ガイド・照準・視点移動でステップごとに誘導。9言語対応。画面右下の<img src="icons/icon-tutorial.svg" width="14" align="absmiddle">ボタンから起動。「<img src="icons/icon-rev.svg" width="14" align="absmiddle">前へ」は進捗があれば現在のレッスン/試験をリセット、無ければ前の項目へ移動 |
...
| PWA自動更新 | 更新は自動適用せず、「アプリが更新されました。更新しますか？」の確認を経てから適用（同意後は全画面ローディング表示）。メニューの「アップデートを確認」（PWAインストール時のみ表示）から手動チェックも可能 |
...
- <strong>Sbs（ステップ実行）</strong>は、**決まった記録を再生・閲覧する**モードです。全ての手・視点はあらかじめ決まっていて、開始後は行き来（1手ずつ進める、最初/最後へジャンプ、好きな速さでオートプレイ）はできても、自分の手を割り込ませたり内容を書き換えたりはできません。向いている用途：チュートリアル、記録した解法の再現（例：ハイスコアの<img src="icons/icon-play.svg" width="14" align="absmiddle">ボタン）、アルゴリズムを1手ずつ確認しながら練習する。
...
| アイコン | 主要ボタンはすべて（多くは[Tabler Icons](https://tabler.io/icons)、MIT）インラインSVGで、プラットフォーム間の描画差異を回避。色は`currentColor`指定にしており、ボタンの状態（通常/ホバー/無効化等）に応じてCSS側から自動追従 |
| ファイル構成 | HTMLファイル単体 |
| リプレイ共有バックエンド | Supabase (PostgreSQL) — [cubew_replay_hub](https://github.com/itachiwalker/cubew_replay_hub)（public）で管理。SHA-256先頭16文字をキーとした短縮リンク、30日で自動削除 |

---

## 📦 PWA（Progressive Web App）対応

- Web App Manifest（`manifest.json`）＋ Service Worker（`sw.js`）によるオフライン対応・ホーム画面インストールに対応
- インストール後もオフラインキャッシュで動作（機内モードでも利用可能）
- 更新は自動適用されない。新しいService Workerは`waiting`状態のまま留まり、アプリ側から`postMessage({type:'SKIP_WAITING'})`を送信するのは、ユーザーが確認ダイアログで明示的に同意した場合のみ（詳細は`sw.js`・`my_cube-base.html`側のコメント参照）
- Service Worker仕様上、アプリを完全に終了している間は、待機中の更新がブラウザによって自動的に有効化されることがある（アプリ側のコードでは防止できない）。この場合、次回起動時に「アプリは前回終了時に自動的に最新版へ更新されました」という通知が一度だけ表示される
- 手動での更新チェックは、メニューの「アップデートを確認」から可能（PWA・通常のWebタブいずれでも表示）
...
cubew_replay_hub/   (public) ← リプレイ共有バックエンド（Supabaseでホスティング）
```
...
ハイスコアのリプレイ共有機能のバックエンド。Supabase (PostgreSQL) のテーブル定義・RLS・RPC関数のマイグレーションのみを管理し、GitHub連携により自動デプロイされる。認証情報は一切含まれない（セキュリティはPostgres側の権限設計そのものに依っており、SQLの中身を隠すことに依存していないため公開可能）。
